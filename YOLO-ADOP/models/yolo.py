...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Decoupled_Detect, Segment, Decoupled_AUX_Detect)):
...
class Decoupled_AUX_Detect(nn.Module):
    # YOLOv5 Detect head for detection models
    stride = None  # strides computed during build
    dynamic = False  # force grid reconstruction
    export = False  # export mode
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.no = nc + 5  # number of outputs per anchor  =15
        ...
        self.m2_stem = nn.ModuleList(Conv(x, x, 1) for x in ch)  # stem conv
        self.m2_cls = nn.ModuleList(nn.Sequential(Conv(x, x, 3), nn.Conv2d(x, self.na * self.nc, 1)) for x in ch)  # cls conv
        self.m2_reg_conf = nn.ModuleList(Conv(x, x, 3) for x in ch)  # reg_conf stem conv
        self.m2_reg = nn.ModuleList(nn.Conv2d(x, self.na * 4, 1) for x in ch)  # reg conv
        self.m2_conf = nn.ModuleList(nn.Conv2d(x, self.na * 1, 1) for x in ch)  # conf conv
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            '''x[i + self.nl] = self.m2_stem[i](x[i + self.nl])  # conv

            bs2, _2, ny2, nx2 = x[i + self.nl].shape
            x_cls2 = self.m2_cls[i](x[i + self.nl]).view(bs2, self.na, self.nc, ny2, nx2).permute(0, 1, 3, 4, 2).contiguous()
            x_reg_conf2 = self.m2_reg_conf[i](x[i + self.nl])
            x_reg2 = self.m2_reg[i](x_reg_conf2).view(bs2, self.na, 4, ny2, nx2).permute(0, 1, 3, 4, 2).contiguous()
            x_conf2 = self.m2_conf[i](x_reg_conf2).view(bs2, self.na, 1, ny2, nx2).permute(0, 1, 3, 4, 2).contiguous()
            x[i + self.nl] = torch.cat([x_reg2, x_conf2, x_cls2], dim=4)'''
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Decoupled_Detect, Segment, Decoupled_AUX_Detect)):
            ...
            m.stride = m.stride[:len(m.stride) // 2]
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # TODO: channel, gw, gd
        elif m in {Detect, Decoupled_Detect, Segment, Decoupled_AUX_Detect}:
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * (len(f) // 2)
            if m is Segment:
